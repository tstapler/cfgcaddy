...
class LinkTestCase:
    ...
    def initialize_fixture_directories(self, src_dir: Path, dest_dir: Path, expected_src_dir: Path,
...
def test_link_create(tmp_path: Path, test_case: LinkTestCase):
    ...
    test_case.initialize_fixture_directories(src_dir, dest_dir, expected_src_dir, expected_dest_dir)
    link = Link(Path(src_dir, test_case.link_src), Path(dest_dir, test_case.link_dest))
    assert link.create() == test_case.link_result
    for expected, actual in [(expected_src_dir, src_dir), (expected_dest_dir, dest_dir)]:
